...
const plans = [
  { name: "Free", price: "$0", detail: "For testing the API and managing a small catalog." },
  { name: "Pro", price: "$29", detail: "For growing teams that need webhooks and integrations.", active: true },
  { name: "Business", price: "$99", detail: "For larger catalog operations and advanced sync controls." },
];
...
export default function SettingsPage() {
  ...
  return (
    <PageShell className="space-y-6 pb-10">
      ...
      <div className={cn("rounded-[1.5rem] border p-4 text-sm", dirty ? "border-amber-200 bg-amber-50 text-amber-800" : "border-emerald-200 bg-emerald-50 text-emerald-800")}>
        {dirty
          ? "You have unsaved settings changes. Click Save settings to keep them."
          : savedAt
            ? `Settings saved. Last updated ${new Date(savedAt).toLocaleString()}.`
            : "Settings are using the default workspace configuration."}
      </div>

      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatusMetric icon={Building2} label="Workspace" value="Configured" helper={settings.organizationName} />
        ...
        <div className="space-y-6">
          <Card className="rounded-[2rem] border-border/80 bg-card/95 shadow-xl shadow-slate-950/5">
            <CardHeader className="border-b bg-gradient-to-br from-card to-muted/25">
              <CardTitle className="flex items-center gap-2 text-xl"><Building2 className="h-5 w-5" /> Organization profile</CardTitle>
          ...
          <Card className="rounded-[2rem] border-border/80 bg-card/95 shadow-xl shadow-slate-950/5">
            <CardHeader className="border-b bg-gradient-to-br from-card to-muted/25">
              <CardTitle className="flex items-center gap-2 text-xl"><BellRing className="h-5 w-5" /> Operational alerts</CardTitle>
          ...
          <Card className="rounded-[2rem] border-border/80 bg-card/95 shadow-xl shadow-slate-950/5">
            <CardHeader className="border-b bg-gradient-to-br from-card to-muted/25">
              <CardTitle className="flex items-center gap-2 text-xl"><LockKeyhole className="h-5 w-5" /> Security and developer defaults</CardTitle>
        ...
        <aside className="space-y-6">
          <Card className="overflow-hidden rounded-[2rem] border-0 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 text-white shadow-xl shadow-slate-950/10">
            <CardContent className="p-6">
              ...
              <p className="mt-2 text-sm leading-6 text-white/70">Your workspace settings now persist in this browser. Backend persistence can be added later when a settings API exists.</p>
              <div className="mt-6 h-2 rounded-full bg-white/10"><div className="h-full rounded-full bg-white" style={{ width: `${readinessScore}%` }} /></div>
              ...
              <Button type="button" onClick={restoreDefaults} variant="secondary" className="mt-5 w-full rounded-xl bg-white/10 text-white hover:bg-white/15">
                Restore defaults
              </Button>
            </CardContent>
          </Card>
          ...
          <Card className="rounded-[2rem] border-border/80 bg-card/95 shadow-xl shadow-slate-950/5">
            <CardHeader><CardTitle className="flex items-center gap-2 text-lg"><Users className="h-5 w-5" /> Team access</CardTitle><p className="text-sm text-muted-foreground">Current workspace members and invitations.</p></CardHeader>
            <CardContent className="space-y-3">
              {teamMembers.map((member) => (
                <div key={member.email} className="flex items-center justify-between gap-3 rounded-2xl border bg-background/70 p-3">
                  <div className="min-w-0"><p className="truncate text-sm font-medium">{member.name}</p><p className="truncate text-xs text-muted-foreground">{member.email}</p></div>
                  <div className="shrink-0 text-right"><Badge variant={member.status === "Active" ? "default" : "secondary"} className="rounded-full">{member.role}</Badge><p className="mt-1 text-xs text-muted-foreground">{member.status}</p></div>
                </div>
              ))}
          ...
          <Card className="rounded-[2rem] border-border/80 bg-card/95 shadow-xl shadow-slate-950/5">
            <CardHeader><CardTitle className="flex items-center gap-2 text-lg"><CreditCard className="h-5 w-5" /> Billing</CardTitle><p className="text-sm text-muted-foreground">Plan design for the MVP pricing model.</p></CardHeader>
            <CardContent className="space-y-3">
              {plans.map((plan) => (
                <div key={plan.name} className={cn("rounded-2xl border bg-background/70 p-4", plan.active && "border-primary/30 bg-primary/5")}>
                  <div className="flex items-center justify-between gap-3"><h3 className="font-semibold">{plan.name}</h3>{plan.active && <Badge className="rounded-full"><Crown className="mr-1 h-3 w-3" /> Current</Badge>}</div>
...
function ToggleRow({ icon: Icon, label, description, checked, onChange }: { icon: ComponentType<{ className?: string }>; label: string; description: string; checked: boolean; onChange: (checked: boolean) => void }) {
  return (
    <button type="button" onClick={() => onChange(!checked)} className="flex w-full items-start gap-3 rounded-2xl border bg-background/70 p-4 text-left transition hover:bg-muted/35 focus:outline-none focus:ring-2 focus:ring-ring/30" aria-pressed={checked}>
      <span className={cn("flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl", checked ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground")}><Icon className="h-5 w-5" /></span>
      <span className="min-w-0 flex-1"><span className="block text-sm font-medium">{label}</span><span className="mt-1 block text-xs leading-5 text-muted-foreground">{description}</span></span>
      <span className={cn("mt-1 flex h-6 w-11 shrink-0 items-center rounded-full p-1 transition", checked ? "bg-primary" : "bg-muted")}><span className={cn("h-4 w-4 rounded-full bg-background shadow-sm transition", checked && "translate-x-5")} /></span>
    </button>
  );
}

function AlertSummary({ label, value, ready }: { label: string; value: string; ready: boolean }) {
  return <div className="rounded-2xl border bg-card p-4"><p className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{label}</p><div className="mt-3 flex items-center justify-between gap-3"><p className="text-sm font-semibold">{value}</p><Badge variant={ready ? "default" : "secondary"} className="rounded-full">{ready ? "On" : "Off"}</Badge></div></div>;
}
...
function StatusMetric({ icon: Icon, label, value, helper, tone = "default" }: { icon: ComponentType<{ className?: string }>; label: string; value: string; helper: string; tone?: "default" | "success" | "warning" }) {
  return <Card className="rounded-[1.5rem] border-border/80 bg-card/95 shadow-sm transition hover:-translate-y-0.5 hover:shadow-xl hover:shadow-slate-950/5"><CardContent className="flex items-start justify-between gap-4 p-5"><div className="min-w-0"><p className="text-sm text-muted-foreground">{label}</p><p className="mt-2 truncate text-2xl font-semibold tracking-tight">{value}</p><p className="mt-1 truncate text-xs text-muted-foreground">{helper}</p></div><span className={cn("flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl", tone === "warning" ? "bg-amber-50 text-amber-700" : tone === "success" ? "bg-emerald-50 text-emerald-700" : "bg-primary/10 text-primary")}><Icon className="h-5 w-5" /></span></CardContent></Card>;
}

function DarkCheck({ label, ready }: { label: string; ready: boolean }) {
  return <div className="flex items-center justify-between rounded-2xl bg-white/10 px-3 py-2.5"><span className="flex items-center gap-2 text-white/75">{ready ? <CheckCircle2 className="h-4 w-4" /> : <Clock3 className="h-4 w-4" />}{label}</span><span className="font-medium">{ready ? "Ready" : "Review"}</span></div>;
}
...
function ShortcutCard({ icon: Icon, title, description, href }: { icon: ComponentType<{ className?: string }>; title: string; description: string; href: string }) {
  return <Link href={href} className="group rounded-[1.5rem] border bg-card/95 p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-xl hover:shadow-slate-950/5"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-primary/10 text-primary transition group-hover:bg-primary group-hover:text-primary-foreground"><Icon className="h-5 w-5" /></span><div className="mt-5 flex items-center justify-between gap-3"><h3 className="font-semibold tracking-tight">{title}</h3><ChevronRight className="h-4 w-4 text-muted-foreground transition group-hover:translate-x-1 group-hover:text-foreground" /></div><p className="mt-2 text-sm leading-6 text-muted-foreground">{description}</p></Link>;
}
