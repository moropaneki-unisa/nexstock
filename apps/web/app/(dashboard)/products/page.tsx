...
function ProductTableRow({ product, deletingId, onRequestArchive }: { product: Product; deletingId: string | null; onRequestArchive: (product: Product) => void }) {
  ...
  return (
    <TableRow className="group h-[68px] transition hover:bg-muted/25">
      ...
      <TableCell><span className="inline-flex bg-muted px-2.5 py-1 text-xs font-medium text-muted-foreground">{product.category || "Uncategorized"}</span></TableCell>
...
function ProductThumbnail({ product }: { product: Product }) {
  ...
  if (image) return <div className="h-10 w-10 shrink-0 overflow-hidden border bg-muted"><img src={image} alt={product.name} className="h-full w-full object-cover" /></div>;
  return <div className="flex h-10 w-10 shrink-0 items-center justify-center border bg-muted text-muted-foreground"><ImageIcon className="h-4 w-4" /></div>;
}
...
function CatalogLoading() {
  return <div className="space-y-3 border-t p-4 sm:p-5">{Array.from({ length: 6 }).map((_, index) => <div key={index} className="h-16 animate-pulse bg-muted" />)}</div>;
}
