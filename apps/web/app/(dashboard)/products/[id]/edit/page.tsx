  ...
  return (
    <PageShell className="space-y-6 pb-10">
      <PageHeader
        ...
        description="Update product details using the same structured organization style: supplier cost first, then selling price and stock."
        actions={
          <div className="flex flex-wrap gap-2">
            <Button asChild variant="outline">
              <Link href="/products"><ArrowLeft className="h-4 w-4" />Back to products</Link>
            </Button>
            {product && <Button asChild><Link href={`/products/${product.id}`}><Eye className="h-4 w-4" />View product</Link></Button>}
          </div>
        }
      />
