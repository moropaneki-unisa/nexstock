...
export default function ProductDetailPage() {
  ...
  return (
    <PageShell className="space-y-6 pb-10">
      <PageHeader eyebrow="Product profile" title={cleanText(product.name) || "Product"} description={`Review product identity, pricing in ${baseCurrency}, inventory, images, attributes, suppliers, and stock movement from one profile.`} actions={<div className="flex flex-wrap gap-2"><Button asChild variant="outline" className="rounded-xl bg-background/70"><Link href="/products"><ArrowLeft className="h-4 w-4" />Back to products</Link></Button><Button type="button" variant="outline" onClick={() => setAdjustOpen(true)} className="rounded-xl bg-background/70"><Warehouse className="h-4 w-4" />Adjust stock</Button><Button asChild className="rounded-xl shadow-sm"><Link href={`/products/${product.id}/edit`}><Edit className="h-4 w-4" />Edit product</Link></Button></div>} />
      <section className="overflow-hidden border bg-card/95"><div className="grid divide-y sm:grid-cols-2 sm:divide-x sm:divide-y-0 xl:grid-cols-4"><Metric label="SKU" value={product.sku} /><Metric label="Stock" value={`${product.quantity} units`} /><Metric label={`Price (${priceCurrency})`} value={formatMoney(product.price, priceCurrency)} /><Metric label="Status" value={lowStock ? "Low stock" : product.status || "active"} status /></div></section>
      <section className="grid gap-6 xl:grid-cols-[18rem_1fr]">
        <aside className="space-y-6">
          <section className="overflow-hidden border bg-card/95">
            <div className="relative aspect-square overflow-hidden bg-muted">
              {primaryImage ? <img src={primaryImage} alt={cleanText(product.name) || "Product image"} className="block h-full w-full object-cover" /> : <div className="flex h-full flex-col items-center justify-center text-muted-foreground"><ImageIcon className="h-10 w-10" /><p className="mt-3 text-sm font-medium">No image</p></div>}
...
function Metric({ label, value, status = false }: { label: string; value: string; status?: boolean }) { return <div className="flex min-h-[88px] items-start justify-between gap-4 p-4"><div className="min-w-0"><p className="text-sm text-muted-foreground">{label}</p><p className={cn("mt-2 min-h-7 truncate text-xl font-semibold capitalize leading-7", label === "SKU" && "font-mono text-base normal-case tracking-tight")}>{value}</p></div>{status && <Badge variant={value.toLowerCase().includes("low") ? "destructive" : "default"}>{value}</Badge>}</div>; }
