    ...
    <VerifyEmailLayout selectedPlan={selectedPlan}>
      <section className="border bg-card/95 shadow-sm">
        <div className="p-6 text-center lg:text-left">
          <div className="mx-auto flex h-11 w-11 items-center justify-center bg-primary/10 text-primary lg:mx-0"><MailCheck className="h-5 w-5" /></div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-0 border-t">
          ...
          <Field label="Verification code"><Input {...register("otp", { required: true })} inputMode="numeric" maxLength={6} className="rounded-xl text-center font-mono text-lg tracking-[0.4em]" placeholder="000000" /></Field>
        ...
        <div className="border-t bg-muted/20 px-5 py-4 text-center text-sm text-muted-foreground">Wrong account? <Link href="/signup" className="font-medium text-foreground hover:underline">Create a new account</Link></div>
      </section>
    </VerifyEmailLayout>
  );
}
    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section className="mx-auto grid min-h-[calc(100vh-73px)] w-full max-w-7xl items-center gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[1fr_28rem] lg:px-10">
        <div className="hidden lg:block">
          <p className="inline-flex items-center gap-2 border bg-card/95 px-3 py-2 text-xs font-semibold uppercase tracking-[0.22em] text-muted-foreground"><ShieldCheck className="h-3.5 w-3.5 text-primary" /> Email verification</p>
          ...
          <section className="mt-8 border bg-card/95"><div className="divide-y"><InfoLine label="Confirm account ownership" /><InfoLine label={`${planLabels[selectedPlan]} plan preserved`} /><InfoLine label={selectedPlan === "free" ? "Unlock dashboard access" : "Continue to subscription checkout"} /></div></section>
        </div>
