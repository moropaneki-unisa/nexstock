    ...
    <ResetPasswordLayout>
      <section className="border bg-card/95 shadow-sm">
        <div className="p-6 text-center lg:text-left">
          <div className="mx-auto flex h-11 w-11 items-center justify-center bg-primary/10 text-primary lg:mx-0"><LockKeyhole className="h-5 w-5" /></div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-0 border-t">
          ...
          <Field label="New password"><Input type="password" {...register("password", { required: true })} className="rounded-xl" placeholder="Create a secure password" /></Field>
          <Field label="Confirm password"><Input type="password" {...register("confirmPassword", { required: true })} className="rounded-xl" placeholder="Confirm your password" /></Field>
        ...
        <div className="border-t bg-muted/20 px-5 py-4 text-center text-sm text-muted-foreground">Need a new link? <Link href="/forgot-password" className="font-medium text-foreground hover:underline">Request reset</Link></div>
      </section>
    </ResetPasswordLayout>
  );
}
    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section className="mx-auto grid min-h-[calc(100vh-73px)] w-full max-w-7xl items-center gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[1fr_28rem] lg:px-10">
        <div className="hidden lg:block">
          <p className="inline-flex items-center gap-2 border bg-card/95 px-3 py-2 text-xs font-semibold uppercase tracking-[0.22em] text-muted-foreground"><LockKeyhole className="h-3.5 w-3.5 text-primary" /> Secure password reset</p>
          ...
          <section className="mt-8 border bg-card/95"><div className="divide-y"><InfoLine label="Use at least 8 characters" /><InfoLine label="Old sessions are revoked" /><InfoLine label="Return to sign in after reset" /></div></section>
        </div>
