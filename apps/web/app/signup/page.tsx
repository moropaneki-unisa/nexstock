    ...
    <SignupLayout selectedPlan={selectedPlan} plan={plan}>
      <section className="border bg-card/95 shadow-sm">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-0 border-t">
          ...
          <Field label="Full name"><Input placeholder="Your name" {...register("name", { required: true })} className="rounded-xl" /></Field>
          <Field label="Organization"><Input placeholder="Company name" {...register("orgName", { required: true })} className="rounded-xl" /></Field>
          <Field label="Email"><Input placeholder="you@company.com" {...register("email", { required: true })} className="rounded-xl" /></Field>
          <Field label="Password"><Input type="password" placeholder="Create a secure password" {...register("password", { required: true })} className="rounded-xl" /></Field>
    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section className="mx-auto grid min-h-[calc(100vh-73px)] w-full max-w-7xl items-center gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[1fr_28rem] lg:px-10">
        <div className="hidden lg:block">
          <p className="inline-flex items-center gap-2 border bg-card/95 px-3 py-2 text-xs font-semibold uppercase tracking-[0.22em] text-muted-foreground"><ShieldCheck className="h-3.5 w-3.5 text-primary" /> Production workspace setup</p>
          ...
          <section className="mt-8 border bg-card/95">
            ...
            <div className="flex items-center justify-between border-t bg-muted/25 px-5 py-4">
              <div className="flex items-center gap-3">
                ...
                <div><p className="text-sm font-medium">Selected plan: {plan.name}</p><p className="text-xs text-muted-foreground">{plan.price} · {plan.helper}</p></div>
              </div>
...
function MiniMetric({ icon: Icon, label, value }: { icon: any; label: string; value: string }) { return <div className="p-4"><Icon className="h-4 w-4 text-primary" /><p className="mt-3 text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{label}</p><p className="mt-1 text-sm font-semibold text-foreground">{value}</p></div>; }
function Readiness({ label }: { label: string }) { return <div className="flex items-center justify-between px-4 py-3 text-sm"><span className="flex items-center gap-2"><CheckCircle2 className="h-3.5 w-3.5 text-emerald-600" />{label}</span><span className="text-xs text-muted-foreground">Ready</span></div>; }
