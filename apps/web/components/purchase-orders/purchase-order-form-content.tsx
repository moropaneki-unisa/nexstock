      ...
      <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_20rem]">
        <Card className="min-w-0 overflow-hidden py-0">
          ...
          <CardContent className="grid gap-4 p-4 sm:grid-cols-2 xl:grid-cols-3">
            <Field label="Supplier" required><Select value={form.supplierId || "none"} onValueChange={(value) => changeSupplier(value === "none" ? "" : value)} disabled={editing}><AppSelectTrigger><SelectValue placeholder="Choose supplier" /></AppSelectTrigger><AppSelectContent><AppSelectItem value="none">Choose supplier</AppSelectItem>{suppliers.map((supplier) => <AppSelectItem key={supplier.id} value={supplier.id}>{supplier.supplierCode} · {supplier.name}</AppSelectItem>)}</AppSelectContent></Select></Field>
            {editing ? <Field label="Status"><Select value={form.status} onValueChange={(value) => setForm((current) => ({ ...current, status: value as Status }))}><AppSelectTrigger><SelectValue /></AppSelectTrigger><AppSelectContent>{statuses.map((status) => <AppSelectItem key={status} value={status}>{titleCase(status)}</AppSelectItem>)}</AppSelectContent></Select></Field> : null}
          ...
          <CardContent className="grid gap-3 p-4 pb-6 md:hidden">{form.lines.map((line, index) => <LineCard key={index} index={index} line={line} currency={currency} editing={editing} linksLoading={linksLoading} products={productOptions} canRemove={form.lines.length > 1} onProductChange={selectProduct} onUpdate={updateLine} onRemove={removeLine} />)}</CardContent>
          <CardContent className="hidden p-0 pb-4 md:block"><div className="overflow-x-auto"><Table className="min-w-[980px] table-fixed"><TableHeader><TableRow><TableHead className="w-72">Product</TableHead><TableHead className="w-40">Supplier SKU</TableHead><TableHead className="w-64">Description</TableHead><TableHead className="w-28">Qty</TableHead><TableHead className="w-36">Unit cost</TableHead><TableHead className="w-36 text-right">Total</TableHead><TableHead className="w-12" /></TableRow></TableHeader><TableBody>{form.lines.map((line, index) => <LineRow key={index} index={index} line={line} currency={currency} editing={editing} linksLoading={linksLoading} products={productOptions} canRemove={form.lines.length > 1} onProductChange={selectProduct} onUpdate={updateLine} onRemove={removeLine} />)}</TableBody></Table></div></CardContent>
        </Card>
...
function ProductSelect({ value, index, products, editing, linksLoading, onProductChange }: { value: string; index: number; products: Product[]; editing: boolean; linksLoading: boolean; onProductChange: (index: number, productId: string) => void }) { return <Select value={value || "none"} onValueChange={(value) => onProductChange(index, value === "none" ? "" : value)} disabled={editing || linksLoading}><AppSelectTrigger><SelectValue placeholder={linksLoading ? "Loading..." : "Choose product"} /></AppSelectTrigger><AppSelectContent><AppSelectItem value="none">Choose product</AppSelectItem>{products.map((product) => <AppSelectItem key={product.id} value={product.id}>{productLabel(product)}</AppSelectItem>)}</AppSelectContent></Select> }
function LineCard({ index, line, currency, editing, linksLoading, products, canRemove, onProductChange, onUpdate, onRemove }: { index: number; line: Line; currency: string; editing: boolean; linksLoading: boolean; products: Product[]; canRemove: boolean; onProductChange: (index: number, productId: string) => void; onUpdate: (index: number, patch: Partial<Line>) => void; onRemove: (index: number) => void }) { return <div className="grid min-w-0 gap-3 rounded-lg border p-3"><div className="flex items-center justify-between"><p className="text-sm font-medium">Line {index + 1}</p>{!editing ? <Button type="button" variant="ghost" size="icon" className="size-8" onClick={() => onRemove(index)} disabled={!canRemove}><Trash2Icon className="size-4" /></Button> : null}</div><Field label="Product"><ProductSelect value={line.productId} index={index} products={products} editing={editing} linksLoading={linksLoading} onProductChange={onProductChange} /></Field><div className="grid min-w-0 gap-3 sm:grid-cols-2"><Field label="Supplier SKU"><Input value={line.supplierSku} onChange={(event) => onUpdate(index, { supplierSku: event.target.value })} disabled={editing} /></Field><Field label="Description"><Input value={line.description} onChange={(event) => onUpdate(index, { description: event.target.value })} disabled={editing} /></Field><Field label="Qty"><Input type="number" min="1" value={line.quantityOrdered} onChange={(event) => onUpdate(index, { quantityOrdered: event.target.value })} disabled={editing} /></Field><Field label="Unit cost"><Input type="number" min="0" step="0.01" value={line.unitCost} onChange={(event) => onUpdate(index, { unitCost: event.target.value })} disabled={editing} /></Field></div><div className="flex items-center justify-between rounded-md bg-muted/40 px-3 py-2 text-sm"><span className="text-muted-foreground">Line total</span><span className="font-medium">{formatMoney(numberValue(line.quantityOrdered) * numberValue(line.unitCost), currency)}</span></div></div> }
function LineRow(props: React.ComponentProps<typeof LineCard>) { const line = props.line; return <TableRow><TableCell className="w-72 min-w-0 max-w-72"><ProductSelect value={line.productId} index={props.index} products={props.products} editing={props.editing} linksLoading={props.linksLoading} onProductChange={props.onProductChange} /></TableCell><TableCell className="w-40 min-w-0 max-w-40"><Input value={line.supplierSku} onChange={(event) => props.onUpdate(props.index, { supplierSku: event.target.value })} disabled={props.editing} /></TableCell><TableCell className="w-64 min-w-0 max-w-64"><Input value={line.description} onChange={(event) => props.onUpdate(props.index, { description: event.target.value })} disabled={props.editing} /></TableCell><TableCell className="w-28"><Input type="number" min="1" value={line.quantityOrdered} onChange={(event) => props.onUpdate(props.index, { quantityOrdered: event.target.value })} disabled={props.editing} /></TableCell><TableCell className="w-36"><Input type="number" min="0" step="0.01" value={line.unitCost} onChange={(event) => props.onUpdate(props.index, { unitCost: event.target.value })} disabled={props.editing} /></TableCell><TableCell className="w-36 text-right font-medium">{formatMoney(numberValue(line.quantityOrdered) * numberValue(line.unitCost), props.currency)}</TableCell><TableCell className="w-12">{!props.editing ? <Button type="button" variant="ghost" size="icon" onClick={() => props.onRemove(props.index)} disabled={!props.canRemove}><Trash2Icon className="size-4" /></Button> : null}</TableCell></TableRow> }
