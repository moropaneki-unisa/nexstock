      ...
      <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_20rem]">
        <Card className="min-w-0 overflow-hidden py-0">
          ...
          <CardContent className="grid gap-4 p-4 sm:grid-cols-2 xl:grid-cols-3">
            <Field label="Supplier" required><Select value={form.supplierId || "none"} onValueChange={(value) => changeSupplier(value === "none" ? "" : value)} disabled={editing}><SelectTrigger className="w-full min-w-0 [&>span]:truncate"><SelectValue placeholder="Choose supplier" /></SelectTrigger><SelectContent><SelectItem value="none">Choose supplier</SelectItem>{suppliers.map((supplier) => <SelectItem key={supplier.id} value={supplier.id}><span className="block max-w-80 truncate">{supplier.supplierCode} · {supplier.name}</span></SelectItem>)}</SelectContent></Select></Field>
...
function ProductSelect({ value, index, products, editing, linksLoading, onProductChange }: { value: string; index: number; products: Product[]; editing: boolean; linksLoading: boolean; onProductChange: (index: number, productId: string) => void }) { return <Select value={value || "none"} onValueChange={(value) => onProductChange(index, value === "none" ? "" : value)} disabled={editing || linksLoading}><SelectTrigger className="w-full min-w-0 [&>span]:truncate"><SelectValue placeholder={linksLoading ? "Loading..." : "Choose product"} /></SelectTrigger><SelectContent className="max-w-[min(32rem,var(--radix-select-trigger-width))]"><SelectItem value="none">Choose product</SelectItem>{products.map((product) => <SelectItem key={product.id} value={product.id}><span className="block max-w-[28rem] truncate">{productLabel(product)}</span></SelectItem>)}</SelectContent></Select> }
