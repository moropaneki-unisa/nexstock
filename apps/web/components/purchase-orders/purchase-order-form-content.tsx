...
type LineForm = { productId: string; supplierSku: string; description: string; quantityOrdered: string; unitCost: string; notes: string }
type FormState = { supplierId: string; status: PurchaseOrderStatus; expectedAt: string; notes: string; lines: LineForm[] }

const emptyLine: LineForm = { productId: "", supplierSku: "", description: "", quantityOrdered: "1", unitCost: "0", notes: "" }
...
export function PurchaseOrderFormContent({ purchaseOrderId }: { purchaseOrderId?: string }) {
  ...
  const currency = normalizeCurrency(selectedSupplier?.currency)
  ...
  React.useEffect(() => {
    async function load() {
      ...
      try {
        ...
        const activeSuppliers = supplierList.filter((supplier) => supplier.status !== "archived")
        ...
        if (existing) {
          ...
          setForm({
            ...
            lines: existing.lines?.length ? existing.lines.map((line) => ({ productId: line.productId, supplierSku: line.supplierSku || "", description: line.description || "", quantityOrdered: String(line.quantityOrdered || 1), unitCost: String(line.unitCost ?? 0), notes: line.notes || "" })) : [{ ...emptyLine }],
          })
        } else {
  ...
  function selectProduct(index: number, productId: string) {
    ...
    const fallbackCost = product?.cost ?? product?.costPrice ?? product?.price ?? 0
    updateLine(index, { productId, description: product?.name || "", unitCost: String(fallbackCost) })
  }
      ...
      if (purchaseOrderId) {
        ...
        const result = await apiFetch<PurchaseOrder>("/api/purchase-orders", {
          ...
          body: JSON.stringify({
            ...
            lines: validLines.map((line) => ({ productId: line.productId, supplierSku: clean(line.supplierSku), description: clean(line.description), quantityOrdered: Number(line.quantityOrdered), unitCost: Number(line.unitCost), notes: clean(line.notes) })),
          }),
        })
    ...
    <div className="@container/main flex flex-1 flex-col gap-4 p-4 md:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">{order ? `${order.poNumber} · update status, expected date, and notes.` : "Create a purchase order from supplier and product lines."}</p>
        </div>
        ...
        <div className="grid gap-4">
          <Card>
            ...
            <CardContent className="grid gap-4 md:grid-cols-2">
              <label className="grid gap-2"><Label>Supplier</Label><Select value={form.supplierId} onValueChange={(value) => setForm((current) => ({ ...current, supplierId: value }))} disabled={Boolean(purchaseOrderId)}><SelectTrigger className="w-full"><SelectValue placeholder="Choose supplier" /></SelectTrigger><SelectContent>{suppliers.map((supplier) => <SelectItem key={supplier.id} value={supplier.id}>{supplier.supplierCode} · {supplier.name}</SelectItem>)}</SelectContent></Select></label>
          ...
          <Card>
            <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><CardTitle>Order lines</CardTitle><CardDescription>{purchaseOrderId ? "Lines are locked after creation by the current API. Create a new PO to change product lines." : "Add at least one product line."}</CardDescription></div>{!purchaseOrderId ? <Button type="button" variant="outline" size="sm" onClick={addLine}><PlusIcon className="size-4" />Add line</Button> : null}</CardHeader>
            <CardContent>
              <div className="overflow-hidden rounded-xl border">
                <Table>
                  <TableHeader><TableRow><TableHead>Product</TableHead><TableHead>Description</TableHead><TableHead className="w-28">Qty</TableHead><TableHead className="w-32">Unit cost</TableHead><TableHead className="w-32 text-right">Total</TableHead><TableHead className="w-12" /></TableRow></TableHeader>
                  <TableBody>
                    {form.lines.map((line, index) => <TableRow key={index}>
                      <TableCell className="min-w-56"><Select value={line.productId} onValueChange={(value) => selectProduct(index, value)} disabled={Boolean(purchaseOrderId)}><SelectTrigger className="w-full"><SelectValue placeholder="Choose product" /></SelectTrigger><SelectContent>{products.map((product) => <SelectItem key={product.id} value={product.id}>{product.sku ? `${product.sku} · ` : ""}{product.name}</SelectItem>)}</SelectContent></Select></TableCell>
