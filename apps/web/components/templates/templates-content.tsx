...
import { ChevronDownIcon, EditIcon, FileTextIcon, Loader2Icon, MailIcon, PenLineIcon, PlusIcon, RefreshCwIcon, Trash2Icon } from "lucide-react"
...
function NewTemplateDropdown({ kindFilter }: { kindFilter?: "pdf" | "email" }) {
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button size="sm"><PlusIcon className="size-4" />New template<ChevronDownIcon className="size-4" /></Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-64">
        ...
        {kindFilter !== "email" ? (
          <DropdownMenuItem asChild>
            <Link href="/settings/templates/new/editor?kind=pdf" className="flex items-start gap-2">
              <FileTextIcon className="mt-0.5 size-4" />
              <span className="grid gap-0.5"><span className="font-medium">PDF template</span><span className="text-xs text-muted-foreground">Design a document/PDF layout</span></span>
            </Link>
          </DropdownMenuItem>
        ) : null}
        {kindFilter !== "pdf" ? (
          <DropdownMenuItem asChild>
            <Link href="/settings/templates/new/editor?kind=email" className="flex items-start gap-2">
              <MailIcon className="mt-0.5 size-4" />
              <span className="grid gap-0.5"><span className="font-medium">Email template</span><span className="text-xs text-muted-foreground">Design subject, recipient, and email body</span></span>
            </Link>
          </DropdownMenuItem>
        ) : null}
      </DropdownMenuContent>
    ...
    <div className="@container/main flex flex-1 flex-col gap-2">
      <div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
        <div className="flex flex-col gap-3 px-4 sm:flex-row sm:items-center sm:justify-between lg:px-6">
          ...
          <div className="flex flex-wrap gap-2"><Button asChild variant="outline" size="sm"><Link href="/settings/templates/pdf"><FileTextIcon className="size-4" />PDF</Link></Button><Button asChild variant="outline" size="sm"><Link href="/settings/templates/email"><MailIcon className="size-4" />Email</Link></Button><Button variant="outline" size="sm" onClick={() => void loadTemplates()} disabled={running || loading}>{running ? <Loader2Icon className="size-4 animate-spin" /> : <RefreshCwIcon className="size-4" />}Refresh</Button><NewTemplateDropdown kindFilter={kindFilter} /></div>
        </div>
