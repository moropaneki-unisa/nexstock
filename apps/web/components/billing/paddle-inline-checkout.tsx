...
import { AlertCircle, ArrowRight, CheckCircle2, CreditCard, ExternalLink, Loader2, RefreshCcw, ShieldCheck, Sparkles } from "lucide-react";
          ...
          <div className="flex h-full flex-col justify-between gap-10">
            <div>
              ...
              <h1 className="mt-8 max-w-xl text-4xl font-black tracking-[-0.055em] sm:text-5xl">
                Review your {plan.name} subscription.
              </h1>
              <p className="mt-5 max-w-lg text-base leading-7 text-white/68">
                Confirm your plan, copy the test card details, then continue to Lemon Squeezy hosted checkout.
              </p>
            </div>
          ...
          <div className="p-6 sm:p-8">
            <div className="rounded-3xl bg-slate-50 p-5">
              <div className="flex items-end gap-2">
                <span className="text-5xl font-black tracking-[-0.07em]">{plan.price.split("/")[0]}</span>
            ...
            <div className="mt-6 rounded-3xl border border-amber-200 bg-amber-50 p-4 text-amber-950">
              <div className="flex items-start gap-3">
                <AlertCircle className="mt-0.5 h-5 w-5 shrink-0" />
                <div>
                  <p className="font-bold">Use test card details only.</p>
                  <p className="mt-1 text-sm leading-6">Do not use a real card while NexStock is still in implementation and payment testing.</p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 space-y-3">
              <Button onClick={startCheckout} className="w-full rounded-2xl py-6 font-bold" disabled={status === "starting" || status === "verifying" || status === "success"}>
                {status === "starting" ? <><Loader2 className="h-4 w-4 animate-spin" /> Creating checkout...</> : <>Continue to Lemon Squeezy <ExternalLink className="h-4 w-4" /></>}
              </Button>
              {reference && <Button type="button" variant="outline" onClick={() => verifyTransaction(reference)} className="w-full rounded-2xl py-6 font-bold" disabled={status === "verifying" || status === "success"}>{status === "verifying" ? <><Loader2 className="h-4 w-4 animate-spin" /> Verifying...</> : <><RefreshCcw className="h-4 w-4" />Verify payment</>}</Button>}
