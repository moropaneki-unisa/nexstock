...
export function NexStockDashboard() {
  ...
  const guidance = React.useMemo(() => {
    ...
    if (outOfStockProducts.length) items.push({ title: `${outOfStockProducts.length} out-of-stock product${outOfStockProducts.length === 1 ? "" : "s"}`, description: "Review suppliers or create purchase orders before sales are affected.", tone: "warning", href: "/products" })
    ...
    if (!items.length) items.push({ title: "Inventory position looks stable", description: "No urgent stock or purchasing issues were detected from current records.", tone: "success", href: "/dashboard" })
  ...
  if (loading) {
    return <div className="nexstock-shell flex flex-1 items-center justify-center p-6"><div className="nexstock-card flex items-center gap-3 rounded-xl px-5 py-4 text-sm text-muted-foreground"><Loader2Icon className="size-5 animate-spin text-primary" />Loading inventory dashboard...</div></div>
  }
  ...
  return (
    <div className="nexstock-shell flex flex-1 flex-col gap-5 p-4 pt-0 lg:gap-6 lg:p-6 lg:pt-0">
      ...
      <section className="nexstock-hero relative overflow-hidden rounded-2xl border p-5 shadow-sm sm:p-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            <div className="inline-flex items-center rounded-md border border-white/15 bg-white/10 px-2.5 py-1 text-xs font-medium text-white/75">{organization?.name || "NexStock workspace"}</div>
            <h1 className="mt-4 font-heading text-2xl font-semibold tracking-tight text-white sm:text-3xl">Inventory operations</h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-white/70">Monitor stock, supplier coverage, purchase orders, and product activity from one operational dashboard.</p>
            <div className="mt-5 flex flex-wrap gap-2">
              ...
              <Button asChild variant="outline" className="border-white/20 bg-white/5 text-white hover:bg-white/10 hover:text-white"><Link href="/purchase-orders/new"><ClipboardListIcon className="size-4" />New purchase order</Link></Button>
          ...
          <div className="grid min-w-[17rem] gap-3 rounded-xl border border-white/10 bg-white/8 p-4">
      ...
      <section className="grid gap-4 xl:grid-cols-[1.35fr_0.65fr]">
        <DashboardCard><CardHeader><CardTitle>Inventory by category</CardTitle><CardDescription>Stock quantity and low-stock alerts across your largest product categories.</CardDescription></CardHeader><CardContent><ChartContainer config={inventoryConfig} className="min-h-[280px] w-full"><BarChart data={stockChart} accessibilityLayer><CartesianGrid vertical={false} strokeDasharray="4 4" /><XAxis dataKey="category" tickLine={false} axisLine={false} tickMargin={8} /><YAxis tickLine={false} axisLine={false} tickMargin={8} /><ChartTooltip content={<ChartTooltipContent />} /><Bar dataKey="quantity" fill="var(--color-quantity)" radius={6} /><Bar dataKey="lowStock" fill="var(--color-lowStock)" radius={6} /></BarChart></ChartContainer></CardContent></DashboardCard>
        <DashboardCard><CardHeader><CardTitle>Operational alerts</CardTitle><CardDescription>Items that need attention based on current data.</CardDescription></CardHeader><CardContent className="grid gap-3">{guidance.map((item) => <Link key={item.title} href={item.href} className="group rounded-xl border bg-background/55 p-3 transition-colors hover:bg-muted/70"><div className="flex items-start gap-3"><span className={cn("mt-0.5 rounded-lg p-2", item.tone === "success" ? "bg-emerald-500/10 text-emerald-600" : item.tone === "warning" ? "bg-amber-500/10 text-amber-600" : "bg-primary/10 text-primary")}>{item.tone === "success" ? <CheckCircle2Icon className="size-4" /> : item.tone === "warning" ? <AlertTriangleIcon className="size-4" /> : <ArrowRightIcon className="size-4" />}</span><div><p className="text-sm font-medium">{item.title}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{item.description}</p></div></div></Link>)}</CardContent></DashboardCard>
      </section>

      <section className="grid gap-4 xl:grid-cols-2">
        <DashboardCard><CardHeader><CardTitle>Workspace activity</CardTitle><CardDescription>Products, suppliers, and purchase order records captured in the workspace.</CardDescription></CardHeader><CardContent><ChartContainer config={statusConfig} className="min-h-[260px] w-full"><AreaChart data={operationsChart} accessibilityLayer><CartesianGrid vertical={false} strokeDasharray="4 4" /><XAxis dataKey="month" tickLine={false} axisLine={false} tickMargin={8} /><YAxis tickLine={false} axisLine={false} tickMargin={8} /><ChartTooltip content={<ChartTooltipContent />} /><Area dataKey="products" type="natural" fill="var(--color-products)" fillOpacity={0.22} stroke="var(--color-products)" /><Area dataKey="suppliers" type="natural" fill="var(--color-suppliers)" fillOpacity={0.18} stroke="var(--color-suppliers)" /><Area dataKey="orders" type="natural" fill="var(--color-orders)" fillOpacity={0.14} stroke="var(--color-orders)" /></AreaChart></ChartContainer></CardContent></DashboardCard>
        <DashboardCard><CardHeader><CardTitle>Purchase order status</CardTitle><CardDescription>{money(purchaseValue, baseCurrency)} in purchase order value.</CardDescription></CardHeader><CardContent><ChartContainer config={orderConfig} className="min-h-[260px] w-full"><PieChart accessibilityLayer><ChartTooltip content={<ChartTooltipContent nameKey="status" />} /><Pie data={orderStatusChart} dataKey="value" nameKey="status" innerRadius={58} outerRadius={88} paddingAngle={3}>{orderStatusChart.map((_, index) => <Cell key={index} fill={`var(--chart-${(index % 5) + 1})`} />)}</Pie></PieChart></ChartContainer></CardContent></DashboardCard>
      </section>

      <section className="grid gap-4 xl:grid-cols-[1fr_0.8fr]">
        <DashboardCard><CardHeader><CardTitle>Recent products</CardTitle><CardDescription>Latest products updated in your catalog.</CardDescription></CardHeader><CardContent className="p-0"><Table><TableHeader><TableRow className="hover:bg-transparent"><TableHead>Product</TableHead><TableHead>Category</TableHead><TableHead className="text-right">Stock</TableHead><TableHead>Status</TableHead></TableRow></TableHeader><TableBody>{recentProducts.length ? recentProducts.map((product) => <TableRow key={product.id} className="hover:bg-muted/35"><TableCell><Link href={`/products/${product.id}`} className="font-medium hover:underline">{product.name}</Link><p className="font-mono text-xs text-muted-foreground">{product.sku || "No SKU"}</p></TableCell><TableCell>{product.category || "Uncategorized"}</TableCell><TableCell className="text-right tabular-nums">{numberValue(product.quantity)}</TableCell><TableCell><ProductBadge product={product} /></TableCell></TableRow>) : <TableRow><TableCell colSpan={4} className="h-24 text-center text-muted-foreground">No products yet.</TableCell></TableRow>}</TableBody></Table></CardContent><CardFooter><Button asChild variant="outline" size="sm"><Link href="/products">View products <ArrowRightIcon className="size-4" /></Link></Button></CardFooter></DashboardCard>
        <DashboardCard><CardHeader><CardTitle>Setup progress</CardTitle><CardDescription>Recommended setup path for reliable inventory operations.</CardDescription></CardHeader><CardContent className="grid gap-3"><SetupStep done={Boolean(products.length)} title="Create or import products" href="/products" /><SetupStep done={Boolean(suppliers.length)} title="Add suppliers" href="/suppliers" /><SetupStep done={Boolean(openOrders.length || receivedOrders.length)} title="Create purchase orders" href="/purchase-orders" /><SetupStep done={Boolean(organization?.baseCurrency)} title="Confirm base currency" href="/organization" /></CardContent></DashboardCard>
      </section>
    </div>
  )
}

function DashboardCard({ children }: { children: React.ReactNode }) { return <Card className="nexstock-card rounded-2xl">{children}</Card> }
function HeroFact({ label, value }: { label: string; value: string | number }) { return <div className="flex items-center justify-between gap-4 border-b border-white/10 pb-3 last:border-b-0 last:pb-0"><span className="text-xs font-medium uppercase tracking-[0.14em] text-white/55">{label}</span><span className="font-semibold tabular-nums text-white">{value}</span></div> }
function MetricCard({ icon: Icon, title, value, detail, href, accent }: { icon: React.ComponentType<{ className?: string }>; title: string; value: string | number; detail: string; href: string; accent: "blue" | "cyan" | "violet" | "teal" }) {
  const accentClass = { blue: "bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-300", cyan: "bg-cyan-50 text-cyan-700 dark:bg-cyan-500/10 dark:text-cyan-300", violet: "bg-violet-50 text-violet-700 dark:bg-violet-500/10 dark:text-violet-300", teal: "bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300" }[accent]
  const lineClass = { blue: "bg-blue-600", cyan: "bg-cyan-600", violet: "bg-violet-600", teal: "bg-emerald-600" }[accent]
  return <Card className="nexstock-card group overflow-hidden rounded-2xl transition hover:shadow-md"><div className={cn("h-1", lineClass)} /><CardHeader className="flex flex-row items-start justify-between space-y-0"><div><CardDescription>{title}</CardDescription><CardTitle className="mt-2 text-2xl font-semibold tracking-tight tabular-nums">{value}</CardTitle></div><span className={cn("rounded-xl p-2.5", accentClass)}><Icon className="size-5" /></span></CardHeader><CardFooter className="justify-between text-sm text-muted-foreground"><span>{detail}</span><Link href={href} className="inline-flex items-center gap-1 font-medium text-foreground hover:underline">Open <ArrowRightIcon className="size-3.5 transition group-hover:translate-x-0.5" /></Link></CardFooter></Card>
}
...
function SetupStep({ done, title, href }: { done: boolean; title: string; href: string }) { return <Link href={href} className="group flex items-center justify-between gap-3 rounded-xl border bg-background/55 p-3 transition-colors hover:bg-muted/70"><div className="flex items-center gap-3"><span className={cn("rounded-lg p-2", done ? "bg-emerald-500/10 text-emerald-600" : "bg-muted text-muted-foreground")}>{done ? <CheckCircle2Icon className="size-4" /> : <PackageCheckIcon className="size-4" />}</span><span className="text-sm font-medium">{title}</span></div><ArrowRightIcon className="size-4 text-muted-foreground transition group-hover:translate-x-0.5" /></Link> }
