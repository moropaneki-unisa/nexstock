...
import { ArrowLeftIcon, DatabaseZapIcon, EditIcon, Loader2Icon, Settings2Icon, Trash2Icon, TriangleAlertIcon } from "lucide-react"
  ...
  if (!layout || error) {
    return (
      <div className="@container/main flex flex-1 flex-col gap-4 p-4 md:p-6">
        <Button asChild variant="outline" size="sm" className="w-fit"><Link href="/layouts"><ArrowLeftIcon className="size-4" />Back to layouts</Link></Button>
  ...
  return (
    <div className="@container/main flex flex-1 flex-col gap-4 p-4 md:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <Button asChild variant="ghost" size="sm" className="-ml-2 mb-2"><Link href="/layouts"><ArrowLeftIcon className="size-4" />Layouts</Link></Button>
          ...
          <p className="mt-1 max-w-2xl text-sm text-muted-foreground">{layout.description || "Reusable product structure built from Product Attributes."}</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 @xl/main:grid-cols-2 @5xl/main:grid-cols-4">
        ...
        <Metric title="Attributes" value={fields.length} detail="Fields in this layout" />
        <Metric title="Default" value={layout.isDefault ? "Yes" : "No"} detail="New product default" />
      </div>

      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Settings2Icon className="size-4" />Layout attributes</CardTitle>
          <CardDescription>Attributes assigned to products using this layout.</CardDescription>
        </CardHeader>
        <CardContent>
          {fields.length ? <div className="grid overflow-hidden rounded-xl border md:grid-cols-2 xl:grid-cols-3">{fields.map((field) => <div key={field.key} className="border-b p-4 text-sm transition hover:bg-muted/25 md:border-r"><div className="flex flex-wrap items-center gap-2"><p className="font-medium">{field.label || field.key}</p>{field.required ? <Badge>Required</Badge> : <Badge variant="secondary">Optional</Badge>}</div><p className="mt-1 font-mono text-xs text-muted-foreground">{`{{product.customFields.${field.key}}}`}</p><Badge variant="outline" className="mt-3 capitalize">{field.type || "text"}</Badge></div>)}</div> : <div className="rounded-xl border border-dashed p-8 text-center"><p className="font-medium">No attributes assigned</p><p className="mt-1 text-sm text-muted-foreground">Edit this layout to select Product Attributes.</p><Button asChild className="mt-4" size="sm"><Link href={`/layouts/${layout.id}/edit`}>Edit layout</Link></Button></div>}
        </CardContent>
      </Card>
    </div>
  )
