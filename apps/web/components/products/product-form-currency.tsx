...
export function ProductForm({ product, mode = "create" }: ProductFormProps) {
  ...
  const form = useForm<FormValues>({
    defaultValues: {
      ...
      costCurrency: product?.costCurrency || product?.priceCurrency || "ZAR",
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="grid gap-6 xl:grid-cols-[1fr_22rem]">
      <div className="space-y-6">
        ...
        {activeStep === "review" && <section className="border bg-card/95"><SectionHeader icon={ShieldCheck} title={isEdit ? "Review product changes" : "Review new product"} description="Confirm the product record before saving it to your catalog." badge="Final step" /><div className="grid divide-y border-t md:grid-cols-2 md:divide-x md:divide-y-0"><div className="divide-y"><ReviewLine label="Product name" value={name || "Missing"} /><ReviewLine label="Category" value={category || "Uncategorized"} /><ReviewLine label="Price" value={`${priceCurrency} ${price}`} /><ReviewLine label="Cost" value={cost === undefined ? "Not set" : `${costCurrency} ${cost}`} /><ReviewLine label={`Converted cost (${currency.baseCurrency})`} value={convertedCost === undefined ? "Not set" : `${currency.baseCurrency} ${convertedCost}`} /><ReviewLine label="Stock" value={`${displayedQuantity} units`} /></div><div className="divide-y"><ReviewLine label="Low-stock alert" value={`${lowStockLevel} units`} /><ReviewLine label="Images" value={`${images.length}`} /><ReviewLine label="Custom attributes" value={`${attributeFields.length}`} /><ReviewLine label="Margin" value={calculateMargin(price, convertedCost ?? cost)} /><div className="p-4"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">Description</p><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-foreground">{description || "No description"}</p></div></div></div></section>}
