...
function extractProductSaveUrl(input: RequestInfo | URL) {
...
export function ProductLayoutBridge({ productId }: { productId?: string }) {
    ...
    async function load() {
      ...
      try {
        const [typeResult, fieldResult, productResult] = await Promise.all([
          apiFetch<unknown>("/api/products/types").catch(() => apiFetch<unknown>("/api/product-types").catch(() => [])),
        ...
        const nextTypeId = currentTypeId || defaultTypeId
  ...
  React.useEffect(() => {
    selectedTypeRef.current = types.find((type) => type.id === selectedTypeId) || null
  ...
  React.useEffect(() => {
    ...
    window.fetch = async (input, init) => {
      ...
      const isProductSave =
        ...
        (url === `${apiBase}/api/products` || /^https?:\/\/[^/]+\/api\/products\/[^/]+$/.test(url) || url === "/api/products" || /^\/api\/products\/[^/]+$/.test(url)) &&
  ...
  if (loading) {
    return <div className="px-4 pt-4 md:px-6"><Skeleton className="h-28 rounded-xl" /></div>
  }
      ...
      <Card>
        <CardHeader className="gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <CardDescription>Choose the layout/type for this product. The selected layout is saved with the product and shown on the product details page.</CardDescription>
          </div>
          <Button asChild variant="outline" size="sm"><Link href="/products/types"><Settings2Icon className="size-4" />Manage layouts</Link></Button>
        </CardHeader>
        <CardContent className="grid gap-4 md:grid-cols-[minmax(0,24rem)_1fr] md:items-start">
          <div className="grid gap-2">
            <Select value={selectedTypeId} onValueChange={setSelectedTypeId}>
              <SelectTrigger><SelectValue placeholder="Choose product layout" /></SelectTrigger>
              <SelectContent>
                {types.map((type) => <SelectItem key={type.id} value={type.id}>{type.name}</SelectItem>)}
              </SelectContent>
            </Select>
            {selectedType ? <div className="flex flex-wrap gap-2"><Badge variant="outline">{humanKind(selectedType.kind)}</Badge><Badge variant={selectedType.trackInventory === false ? "secondary" : "default"}>{selectedType.trackInventory === false ? "No stock tracking" : "Stock tracked"}</Badge>{selectedType.isDefault ? <Badge variant="secondary">Default</Badge> : null}</div> : null}
          </div>
          <div className="flex flex-wrap gap-2">
            {selectedFields.length ? selectedFields.map((field) => <Badge key={field.key} variant="secondary">{field.label || field.key}</Badge>) : <span className="text-sm text-muted-foreground">No layout attributes selected yet.</span>}
          </div>
        </CardContent>
      </Card>
    </div>
