        ...
        <div className="grid gap-4 self-start">
          <Card className="gap-0 overflow-hidden py-0">
            <div className="relative aspect-square overflow-hidden bg-muted">
              ...
              {images.length ? <div className="absolute bottom-3 right-3"><ButtonGroup className="rounded-lg bg-background/90 shadow-sm backdrop-blur"><Button type="button" variant="outline" size="icon" onClick={showPreviousImage} disabled={images.length < 2} aria-label="Previous product image" className="size-8 bg-background/90"><ChevronLeftIcon className="size-4" /></Button><ButtonGroupText className="h-8 min-w-16 justify-center border-y bg-background/90 px-2 text-xs tabular-nums">{currentImageIndex + 1} / {images.length}</ButtonGroupText><Button type="button" variant="outline" size="icon" onClick={showNextImage} disabled={images.length < 2} aria-label="Next product image" className="size-8 bg-background/90"><ChevronRightIcon className="size-4" /></Button></ButtonGroup></div> : null}
            </div>
          </Card>
