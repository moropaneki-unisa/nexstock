        ...
        <div className="grid gap-4 self-start xl:sticky xl:top-[calc(var(--header-height)+1rem)] xl:max-h-[calc(100vh-var(--header-height)-2rem)] xl:overflow-y-auto xl:pr-1">
          <Card className="gap-0 overflow-hidden py-0">
            <div className="relative aspect-square overflow-hidden bg-muted">
              ...
              {images.length ? <div className="absolute bottom-3 right-3 opacity-55 transition-opacity duration-200 hover:opacity-100 focus-within:opacity-100"><ButtonGroup className="rounded-lg bg-muted/85 shadow-sm backdrop-blur"><Button type="button" variant="outline" size="icon" onClick={showPreviousImage} disabled={images.length < 2} aria-label="Previous product image" className="size-8 border-muted bg-muted text-white hover:bg-muted/90 hover:text-white disabled:border-muted/50 disabled:bg-muted/50 disabled:text-white/70"><ChevronLeftIcon className="size-4" /></Button><ButtonGroupText className="h-8 min-w-16 justify-center border-y bg-muted/85 px-2 text-xs font-medium tabular-nums text-foreground">{currentImageIndex + 1} / {images.length}</ButtonGroupText><Button type="button" variant="outline" size="icon" onClick={showNextImage} disabled={images.length < 2} aria-label="Next product image" className="size-8 border-muted bg-muted text-white hover:bg-muted/90 hover:text-white disabled:border-muted/50 disabled:bg-muted/50 disabled:text-white/70"><ChevronRightIcon className="size-4" /></Button></ButtonGroup></div> : null}
            </div>
          </Card>
