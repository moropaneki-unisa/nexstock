...
var ApiKeyGuard = (function () {
    function ApiKeyGuard(prisma) {
        this.prisma = prisma;
    }
    ApiKeyGuard.prototype.canActivate = function (ctx) {
        return __awaiter(this, void 0, void 0, function () {
            var req, auth, rawKey, keyHash, apiKey;
            var _a;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        rawKey = auth.slice('Bearer '.length);
                        keyHash = (0, crypto_1.createHash)('sha256').update(rawKey).digest('hex');
                        return [4, this.prisma.apiKey.findFirst({ where: { keyHash: keyHash, revokedAt: null } })];
                    case 1:
                        apiKey = _b.sent();
                        if (!apiKey)
                            throw new common_1.UnauthorizedException('Invalid API key');
                        return [4, this.prisma.apiKey.update({ where: { id: apiKey.id }, data: { lastUsedAt: new Date() } })];
                    case 2:
                        _b.sent();
